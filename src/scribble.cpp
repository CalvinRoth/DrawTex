...
void Scribble::drawEllipse(const QPoint &endPoint){
    ...
    //TODO OutLine Width
    QString addition =  "    \\draw[color = {rgb, 255:" + colorToRGBs() + "},]" + pointToQString(lastPoint) + "ellipse (" +
            QString::number(abs(endPoint.x()-lastPoint.x())) + " and " + QString::number(abs(endPoint.y()-lastPoint.y())) + "); \n";
    texOut += addition;
    state* s = new state(addition, 1);
    s->pb(lastPoint.x());
    s->pb(lastPoint.y());
    s->pb(abs(endPoint.x()-lastPoint.x()));
    s->pb(abs(endPoint.y()-lastPoint.y()));
    s->pb(myPenColor.red());
    s->pb(myPenColor.green());
    s->pb(myPenColor.blue());
    s->pb(myPenWidth);
...
void Scribble::undo(){
    ...
    for(auto it = stateList.begin(); it != stateList.end(); it++){
        QVector<int> ar = (*it).args;
        setPenColor(qRgb(ar[4], ar[5], ar[6]));
        setPenWidth(ar[7]);
        painter.drawEllipse(ar[0], ar[1], ar[2], ar[3]);
    }
    update();
}
